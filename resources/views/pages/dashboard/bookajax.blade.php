    ...
    @if (!empty(optional($booking->student)->role_id == '4'))
        <tr>
            ...
            <th class="text-capitalize">
                <a href="{{ url('tutor/profile') . '/' . optional($booking->tutor)->id }}"
            ...
            <th class="dropdown">
                ...
                @if (
                    $booking->request_refound == '2' ||
                        $booking->request_refound == '1' ||
                        $booking->status == 'Cancelled' ||
                        $booking->status == 'Cancelled By User' ||
                        $booking->status == 'Cancelled By Tutor')
                    <button class="btn student-table-details dropdown-toggle" type="button" id="dropdownMenuButton1"
                ...
                <ul class="dropdown-menu" aria-labelledby="dropdownMenuButton1">
                    @if (Auth::user()->role_id == 5 || Auth::user()->role_id == 6)
                        <li>
                            <a href="{{ url('chat') . '/' . $booking->tutor->id }}" class="dropdown-item">Let’s
                                chat with Tutor</a>
                            <a href="{{ url('chat') . '/' . $booking->student->id }}" class="dropdown-item">Let’s chat
                                with Student</a>
                        </li>
                    @endif
                    ...
                    @else
                        @if ($booking->request_refound != '1' && $booking->request_refound != '2')
                            <li>
                                <a onClick="viewRating(`{{ $booking->uuid }}`)" class="dropdown-item cursor-pointer"
                                    style="cursor: pointer">View Feedback</a>
                            </li>
                            @if (Auth::user()->role_id != 3)
                                <li>
